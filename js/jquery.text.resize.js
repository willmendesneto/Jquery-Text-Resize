...
(function($) {
	$.textResize = function(selector, settings) {
		...
		var obj = $(configSelector); // Iniciando o objeto com base no
		// container informado na inicializacao
		var img = obj.find('img'); // Achando imagem dentro do container
		// inicializado
		var imgDefault = img.width(); 
		// Pegando a altura da imagem
		var fontDefault = obj.css('font-size'); // Pegando o atributo
												// "font-size" do container
												// inicializado
		var currentSize = parseInt(obj.css('font-size')); // Passando o valor
															// "font-size" para
															// um valor @int
		var userClicksReduce = userClicksEnlarge = 0; // Inicializando o valor de quantidade de clicks para aumento com '0'
		...
		// Pegando o valor do COOKIE, caso ele exista no NAVEGADOR insere o
		// valor dele no container especificado
		...
		// Verificacao da configuracao do container onde sera inserido os
		// botoes/links do plugin, verificando a configuracao do plugin
...
/*
 * Finalizando o plugin Jquery Text Resize @name = jquery.text.resize.js @author =
 * Wilson Mendes Neto / Thiago Teles
 */